...
embedded_uart_hpp="$repo_root/tests/fixtures/project_embedded_uart/build/cmake-backend/build/generated/main.hpp"
grep -q -- "-ffreestanding" "$embedded_uart_cmake"
grep -q -- "-fno-exceptions" "$embedded_uart_cmake"
grep -q -- "-fno-rtti" "$embedded_uart_cmake"
grep -q "volatile uint32_t status" "$embedded_uart_hpp"
grep -q "reinterpret_cast<volatile DuduMainUartRegs\\*>" "$embedded_uart_hpp"
if (
    cd "$repo_root/tests/fixtures/project_library"
