...
bash -n "$repo_root/scripts/add-release-asset.sh"
bash -n "$repo_root/scripts/build-deb.sh"
...
compile_and_expect native_string_reference_alias 0
...
compile_and_expect generic_fixed_array_pack 42
compile_and_expect generic_recursive_fixed_array_overload 70
compile_and_expect generic_recursive_outer_array 70
compile_and_expect generic_wrapped_outer_array_overload 70
compile_modules_path_and_expect generic_fixed_array_pack_imported \
    tests/fixtures/project_generic_fixed_array_pack/main.dd 42
...
compile_and_expect pointer_list_indexer_view 42
compile_and_expect shaped_index_assignment_numeric 42
...
compile_and_expect array_view_reslice 70
...
compile_path_and_expect tensor_dogfood_activation_metrics tests/fixtures/tensor_dogfood/activation_metrics_main.dd 42
...
compile_and_expect member_shape_metadata_method 42
...
compile_path_and_expect tensor_dogfood_shape_mask_composition tests/fixtures/tensor_dogfood/shape_mask_composition.dd 42
...
compile_path_and_expect tensor_dogfood_mask_elements tests/fixtures/tensor_dogfood/mask_elements_main.dd 42
...
printf '%s\n' "$cmake_noop_output" | grep -Fq 'Dudu emit modules'
printf '%s\n' "$cmake_noop_output" | grep -Fq 'dirty 0 modules'
printf '%s\n' "$cmake_noop_output" | grep -Fq 'analyze 0 modules'
