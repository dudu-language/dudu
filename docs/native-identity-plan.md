...
- The compiler has a dedicated libclang C-API boundary that collects stable
  ...
  scanning attaches those USRs to declarations parsed from Clang's text AST.
  Both the raw scanner cache and parsed scan cache preserve the identities, and
  stale path-identity caches are schema-invalidated. Focused tests cover aliases,
  classes, methods, overloaded functions, and identity-sidecar round trips.
