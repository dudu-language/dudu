...
| libuuid | utility / systems | typedefed C array API through explicit byte array handoff, constants, compare/copy, link | pass | `scripts/probe_optional.sh` / `uuid_parse.dd` | no |
...
| mutable C string output buffers | C interop | `char *out` APIs with user-owned buffers | planned | Found while probing libuuid `uuid_unparse_lower(..., char *out)`: scanner exposes the parameter as `cstr`, which is too input-string-like for mutable output. |
