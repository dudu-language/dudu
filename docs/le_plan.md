...
13. Language Server And Formatter
   ...
   docs survive transport. `scripts/test_lsp.sh` runs that matrix after the
   older smoke suite. Generic LSP symbol details now retain generic parameter
   lists for Dudu function, method, and class signatures, and explicit native
   C++ template calls are covered for hover, definition, references, signature
   help, semantic tokens, and header documentation.
