...
10. Real Library Stress Tests
   ...
   external sibling repos such as `duduplayground`, `raymarch-dd`,
   `dudu-webserver`, and similar projects are dogfood inputs, not required
   public fixtures; curated, reproducible examples should live in the Dudu repo
   when they become official compatibility checks. `dudu-webserver` is currently
   a useful POSIX/C++ standard-library dogfood repo because it stresses
   multi-file Dudu modules, generated CMake builds, repeated native imports,
   sockets, `poll`, time APIs, environment reads, and C++ string interop without
   wrapper headers.
...
13. Language Server And Formatter
   ...
   fields such as `Enemy.hp`. Member use sites such as `player.hp` and
   `player.move(...)` resolve through the receiver's structured type to the same
   qualified member identity, including when `Player` comes from an imported
   Dudu module. Enum value declaration references use the same qualified
   identity shape, so `Mode.Play` does not include another enum's `Play`
   variant.
   ...
   and code actions that return concrete WorkspaceEdits for missing imports,
   organize imports, and lint quick fixes. The matrix also drives
   `completionItem/resolve` and asserts resolved completion detail and markdown
   docs survive transport. `textDocument/prepareRename` is advertised and
   covered for both an accepted imported Dudu function use and a rejected native
   symbol use. `scripts/test_lsp.sh` runs that matrix after the older smoke
   suite. Generic LSP symbol details now retain generic parameter lists for Dudu
   function, method, and class signatures, and explicit native C++ template
   ...
   line detail summaries. Use-site rename for module-qualified and
   selective-import Dudu symbols now uses the same target identity as
   references, so renaming `math.mix` or `MAX_HP` edits the imported source
   declaration and matching uses without touching unrelated `other_math.mix` or
   `other_entities.MAX_HP` symbols.
