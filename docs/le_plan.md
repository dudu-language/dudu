...
13. Language Server And Formatter
   ...
   fields such as `Enemy.hp`. Member use sites such as `player.hp` resolve
   through the receiver's structured type to that same `Player.hp` identity,
   including when `Player` comes from an imported Dudu module. Enum value
