...
std::string indexed_local_type(const TypeRef& receiver_type, const Expr& index_expr) {
    ...
    case TypeKind::Device:
    case TypeKind::Static:
        return receiver_type.children.empty() ? std::string{}
                                              : indexed_local_type(receiver_type.children.front(),
                                                                   index_expr);
    case TypeKind::Template:
    case TypeKind::FixedArray: {
    ...
    case ExprKind::Unary:
        ...
        if (expr.op == "*") {
            std::string child = trim_copy(
                infer_emitted_local_type(expr.children.front(), locals, local_type_refs,
                                         function_returns));
    ...
    case ExprKind::Index:
        if (expr.children.size() == 2) {
            ...
            const std::string receiver_type =
                infer_emitted_local_type(expr.children[0], locals, local_type_refs,
                                         function_returns);
            if (!receiver_type.empty()) {
