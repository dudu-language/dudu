...
SourceLocation location(const protocol::SourceRange& range, const protocol::SourceRange& fallback) {
...
void append_note(std::vector<CompileNote>& out, const protocol::Diagnostic& diagnostic,
    ...
    out.push_back(
        {.location = location(diagnostic.range, fallback), .message = diagnostic.message});
...
CompileError compile_error_from_macro_diagnostic(const protocol::Diagnostic& diagnostic,
                                                 const protocol::SourceRange& fallback) {
    return CompileError(location(diagnostic.range, fallback), diagnostic.message,
...
CompileError compile_error_from_worker(const protocol::WorkerError& error,
    ...
    return CompileError(location({}, invocation), message,
