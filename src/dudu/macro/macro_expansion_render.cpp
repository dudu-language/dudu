...
void render_group(std::ostringstream& out, std::string_view label,
                  const std::vector<p::GeneratedDeclaration>& declarations, bool show_origins) {
    if (declarations.empty())
        return;
    out << "# " << label << '\n';
    for (const p::GeneratedDeclaration& generated : declarations) {
        if (show_origins && !generated.origin.range.file.empty()) {
            out << "# generated by @" << generated.origin.macro_name << " from "
                << generated.origin.range.file << ':' << generated.origin.range.start.line << ':'
                << generated.origin.range.start.column << '\n';
        }
...
void render_diagnostic(std::ostringstream& out, const p::Diagnostic& diagnostic,
                       std::string_view prefix = {}) {
    const char* severity = diagnostic.severity == p::DiagnosticSeverity::Error     ? "error"
                           : diagnostic.severity == p::DiagnosticSeverity::Warning ? "warning"
                                                                                   : "note";
    out << "# " << prefix << severity;
    if (!diagnostic.code.empty())
        out << '[' << diagnostic.code << ']';
    if (!diagnostic.range.file.empty())
        out << ' ' << diagnostic.range.file << ':' << diagnostic.range.start.line << ':'
            << diagnostic.range.start.column;
    out << ": " << diagnostic.message << '\n';
    for (const p::Diagnostic& note : diagnostic.notes)
        render_diagnostic(out, note, "  ");
}

bool matches_filter(const ExpansionReport::Record& record, std::string_view filter) {
    return filter.empty() || record.macro_name == filter || record.macro_identity == filter ||
           record.macro_identity.ends_with("." + std::string(filter));
}
...
std::string render_expansion_report(const ExpansionReport& report,
    ...
    for (const ExpansionReport::Record& record : report.expansions) {
        if (!matches_filter(record, options.macro_filter))
            continue;
        ++rendered;
        ...
        if (options.show_origins && !record.definition.file.empty())
            out << "# macro defined at " << record.definition.file << ':'
                << record.definition.start.line << ':' << record.definition.start.column << '\n';
        if (options.show_origins && !record.source_declaration.file.empty())
            out << "# source declaration at " << record.source_declaration.file << ':'
                << record.source_declaration.start.line << ':'
                << record.source_declaration.start.column << '\n';
        for (const p::Diagnostic& diagnostic : record.expansion.diagnostics)
            render_diagnostic(out, diagnostic);
        render_group(out, "generated members", record.expansion.members, options.show_origins);
        render_group(out, "generated siblings", record.expansion.siblings, options.show_origins);
        render_group(out, "generated implementations", record.expansion.implementations,
                     options.show_origins);
    ...
    if (rendered == 0)
        out << (options.macro_filter.empty() ? "# no macro expansions\n"
                                             : "# no matching macro expansions\n");
    return out.str();
}
