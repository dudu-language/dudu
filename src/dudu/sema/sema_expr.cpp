...
void check_declared_index_operator_if_any(const FunctionScope& scope, const TypeRef& receiver_type,
    ...
    if (const auto signature = dudu_operator_signature(scope.symbols, op, receiver_type)) {
        (void)signature;
        if (location != nullptr) {
            sema_expr_fail(*location, dudu_operator_no_match_message_for_args(
                                          scope.symbols, op, receiver_type, args,
                                          infer_arg_type_refs(scope, args, location),
                                          "indexed access", label));
        }
    }
}
    ...
    case ExprKind::Name:
        ...
        if (scope.symbols.generic_params.contains(expr.name)) {
            if (!scope.symbols.types.contains(expr.name)) {
                return named_type_ref("usize", type_location);
            }
            if (location != nullptr) {
        ...
        if (expr.children.size() == 2 && !expr_missing(expr.children[0]) &&
            ...
            const TypeRef receiver_member_type =
                hook_receiver.kind == ExprKind::Member
                    ? member_expr_type_ref(scope.symbols, scope.local_type_refs, location,
