...
std::string hover_json(const Document& doc, const std::string& word, const Json* params,
    ...
    if (const std::optional<DecoratorSelection> decorator =
        ...
        if (const std::optional<Symbol> builtin = builtin_decorator_symbol(*decorator)) {
            return symbol_hover_json(*builtin);
        }
        query = decorator->name;
        selected_path = decorator->path;
    }
    ...
    if (has_selection) {
        ...
        if (selection.member_expr) {
            ...
            if (const std::optional<Symbol> member = dudu_member_symbol_for_expr(
                    current, *selection.member_expr, use_location, call)) {
